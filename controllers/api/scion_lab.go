...
func (s *SCIONLabVMController) parseURLParameters(r *http.Request) (string, string, error) {
	...
	if scionLabVMIP == "undefined" {
		log.Printf("IP address cannot be empty. User: %v", userEmail)
...
func (s *SCIONLabVMController) updateDB(svmInfo *SCIONLabVMInfo) error {
	if svmInfo.IsNewUser {
		...
		if err = new_as.Insert(); err != nil {
			log.Printf("Error inserting new AS: %v User: %v, %v", new_as.String(), user, err)
		...
		if err = new_svm.Insert(); err != nil {
			log.Printf("Error inserting new SCIONLabVM. User: %v, %v", user, err)
		...
		if err := svmInfo.SVM.Update(); err != nil {
			log.Printf("Error updating SCIONLabVM Table. User: %v, %v", svmInfo.UserEmail, err)
