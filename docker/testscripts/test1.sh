...
onExit() {
    ...
    if [ ! -z "$restoreGen" ]; then
        rm -rf "$SC/gen-previous-coordinator-test-run"
        mv "$SC/gen" "$SC/gen-previous-coordinator-test-run"
        mv "$restoreGen" "$SC/gen"
    fi
...
# while true; do
#     echo heyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy
#     date
#     sleep 1
# done
...
# VERS=$(ls $SC/gen/ISD1/AS11/br1-11-1/certs/*.crt | sort -r | awk 'NR==1{print $1}' | sed -n 's/^.*ISD1-AS11-V\([0-9]*\)\.crt/\1/p')
# if missingOrDifferentFiles "$SC/gen/ISD1/AS11/br1-11-1/keys/as-sig.seed" ISD1.key ||
#    missingOrDifferentFiles "$SC/gen/ISD1/AS11/br1-11-1/certs/ISD1-AS11-V$VERS.crt" ISD1.crt ||
#    missingOrDifferentFiles "$SC/gen/ISD1/AS11/br1-11-1/certs/ISD1-V$VERS.trc" ISD1.trc ;
# then
...
#     cp "$SC/gen/ISD1/AS11/br1-11-1/keys/as-sig.seed" ISD1.key
#     VERS=$(ls $SC/gen/ISD1/AS11/br1-11-1/certs/*.crt | sort -r | awk 'NR==1{print $1}' | sed -n 's/^.*ISD1-AS11-V\([0-9]*\)\.crt/\1/p')
#     cp "$SC/gen/ISD1/AS11/br1-11-1/certs/ISD1-AS11-V$VERS.crt" ISD1.crt
#     cp "$SC/gen/ISD1/AS11/br1-11-1/certs/ISD1-V$VERS.trc" ISD1.trc
# fi
...
# if [ -d "$SC/gen/ISD1/AS1001" ]; then
#     pushd "$SC/gen/ISD1" >/dev/null
